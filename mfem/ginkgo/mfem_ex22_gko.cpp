...
//  -no-gko, --no-ginkgo : Use the default MFEM solver instead of Ginkgo (for easy comparison).  
//  -d "cuda" : Use the MFEM cuda backend and Ginkgo CudaExecutor.
//  -d "hip" : Use the MFEM hip bakcend and Ginkgo HipExecutor. 
//
//
// MFEM's provided description about `ex2.cpp`:
...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&pa, "-pa", "--partial-assembly", "-no-pa",
                  "--no-partial-assembly", "Enable Partial Assembly (default is true).");
   args.AddOption(&use_ginkgo, "-gko", "--ginkgo", "-no-gko",
                  "--no-ginkgo", "Enable Ginkgo solver (default is true).");
